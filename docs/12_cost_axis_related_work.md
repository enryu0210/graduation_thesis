...
### 1.6 ⚠️ 누락분 상환 (2026-09-15) — **데이터셋 원 논문 2편이 어느 표에도 없었다**

docs/13 §6 의 인용 문헌 15편을 §1~§1.5 와 전수 대조한 결과, **2편이 수집 목록에서 빠져 있었다.**
하필 **주 데이터셋과 외부 검증셋의 원 논문**이라 논문 §데이터셋 절에서 반드시 인용해야 하는 것들이다.
§1.3 이 상환했던 것과 **같은 종류의 누락**이 다시 일어난 셈이다(지표 논문은 꼼꼼히 적으면서
데이터셋 논문은 "당연히 아는 것"으로 여겨 표에 안 적는 패턴).
...
RiskGate-IDS · E-WebGuard 는 §1.4 "확보 실패" 표에 각각 있다.

**⚠️ 이 머신(노트북)의 `docs/thetics/` 실측 (2026-09-15)** — §1.4 의 경고가 또 사실로 확인됐다.

```
docs/thetics/ 에 실제로 있는 파일 : 18편, 주제별 5개 폴더 없음(평면 구조)
→ §1.4 가 2026-08-24 데스크톱에서 받은 14편과 §1.5 의 폴더 재편이 통째로 없다
```
...
- **새 지표 `F1 Efficiency (FE)`**: F1 과 정규화 추론시간의 **가중평균**(가중치 α).
  ...
  하위로 밀린다는 걸 보인다. **원문 정의는 §IV-A Eq.5 — 전문과 해석은 §4.4 에 옮겨 적었다.**
...
### 4.4 FE 지표 — **채택 확정 (2026-09-15)** + 원문 Eq.5 와 해석
...
**→ 채택한다.** docs/13 §1.9 발견 ③ 이 "미결로 남겨 둘 수 없다"고 판정했고(최근접 선행이 실제로
...
**⚠️ 원문 표기에 모호함이 있다 — 우리가 내린 해석을 남긴다.**

문자 그대로 *"정규화 추론시간"* 을 읽으면 **느릴수록 FE 가 커진다.** 지표의 취지와 정반대다.
그런데 같은 논문 Fig.2 설명은 *"α 를 1.00 → 0.98 로 낮추면 추론이 느린 transformer 계열이
순위 하단으로 밀린다"* 고 적는다. 이 서술과 정합하는 해석은 하나뿐이다:

> **`l` 은 "빠를수록 1 에 가까운 속도 점수"다.** `l = (기준 지연) / (해당 방법의 지연)`.
> 정의역이 `[0,1]` 이 아니라 **`(0,1]` 로 0 을 배제**한 것도 나눗셈 형태를 시사한다
> (뺄셈 판본 `1 − t/t_max` 라면 0 이 정상적으로 나온다).

- 우리 구현은 이 해석을 따르고, 기준 지연은 **비교 대상 중 최소 지연**을 쓴다.
- ⚠️ **이 해석은 우리가 추론한 것이지 원문이 명시한 것이 아니다.** 논문에 FE 를 실을 때
  각주로 이 사실을 밝힌다 — 심사에서 "원문과 부호가 반대 아니냐"는 질문이 나올 수 있는 지점이고,
  조용히 넘어가면 우리가 정의를 바꾼 것처럼 보인다.
- α 기본값은 Tasdemir 의 주 시나리오인 **0.98** 을 쓴다(α=1.0 이면 FE 는 F1 로 퇴화한다 —
  구현의 방향 검증 테스트가 이 성질을 쓴다).

**⚠️ 대조할 때의 한계** — 저쪽 FE 수치와 **직접 비교하지 말 것.** `l` 이 상대값이라
FE 는 "그 비교군 안에서의 순위"만 의미가 있다. 비교군이 다르면(저쪽 35개 방법 vs 우리 모델들)
같은 모델도 FE 가 달라진다. 우리는 **우리 모델들끼리의 재랭킹**에만 쓴다.

---

## 5. 열린 결정

- [x] **FE(F1 Efficiency) 지표 채택 여부** → **채택 확정(2026-09-15, §4.4).** 원문 Eq.5 를 PDF 로
      확인하고 `l` 의 모호함에 대한 해석까지 §4.4 에 고정했다. 구현은 `src/eval/metrics.py` 한 곳.
