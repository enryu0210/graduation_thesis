...
**열린 결정 (전체 파일 확보 후 실측으로 판단)**
- [ ] 248 과 88 의 동시 발생 구조 측정 — 88 이 붙은 요청에 248 도 항상 붙는가, 248 단독 요청은 몇 건인가
- [ ] 결과에 따라 (가) 248 을 CmdI 로 합친다(242→`CodeInj` 와 같은 "상위 범주" 논리) / (나) 248 단독은 제외 — 중 택1
- [ ] 멀티라벨 규칙 2(§2.5)의 "대상 4클래스"에 248 을 포함할지 함께 확정
