...
- [ ] `any-misclass` → **ASR(Attack Success Rate)** 로 개명 + 정의 병기(§1.9 발견 ②).
      ⚠️ 2026-09-15 조사: 대상은 `src/attacks/run_evasion.py`(`any_misclass` 함수 + `anymis_clean`·
      `anymis_mutated` 산출 키)와 `src/attacks/compare_evasion.py`(그림 라벨·축·콘솔 출력) **2파일**이다.
      ⚠️ **기존 산출 JSON 의 키를 바꾸면 2기 결과를 다시 읽을 수 없다** → 새 이름을 병기하거나
      로더에 하위 호환을 두는 쪽으로 설계할 것. AUA·AVGQ(아래)와 **한 작업으로 묶는 것이 낫다** —
      같은 두 파일을 두 번 건드리게 된다
...
- [ ] **AUA(accuracy under attack) · AVGQ(평균 질의 수) 신설** — §1.10 발견 ④. RQ2·RQ3 대조 성립용.
      **AUA 는 이미 재고 있는 값이라 이름만 맞추면 되고**, AVGQ 는 **공격자 비용**이라 F7(비용 기반 공격)의
      교환비를 성립시킨다. ⚠️ AVGQ 는 우리 변형이 질의 기반이 아니라 **변형 강도 k 축**이므로
      "성공까지 필요한 최소 k" 로 번역해야 한다 — 정의를 논문에 명시할 것(그대로 AVGQ 라 부르면 오해된다)
