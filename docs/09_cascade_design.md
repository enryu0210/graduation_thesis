# Phase 10 — 캐스케이드 탐지기 설계/결정 문서 (RGB CNN 1차 + char-CNN 2차)
...
착수 동기: *"char-CNN 의 탐지 정확도와 RGB CNN 의 속도 장점을 결합할 수 없나?"*
...
|---|---|---|---|---|
| gray CNN | 0.9714 / csicnorm 0.967 | 0.9609 | **92,466 /s** | 정확도 하위 · **비용 최저** |
| RGB CNN rb/cc/bd | 0.9836 | 0.9775 | **81,964 /s** | RGB 로 격차 좁힘, 여전히 4위 |
| char-CNN | **0.9969** / csicnorm 0.995 | **0.9957** | 6,010 /s | **정확도 최상위** · **13.6배 느림** |

즉 RGB CNN 의 실질적 강점은 "탐지 우월성"이 아니라 **비용**이다. docs/04 §5 는 이를
*"제안 기법이 실제로 이기는 축은 정확도가 아니라 처리량 … Pareto front 는 LogReg → RGB CNN →
char-CNN"* 으로 정리했고, docs/04 §8 상보성 분석도 "clean 탐지 상보성 이득 없음"을 확인했다.

**이 Phase 의 위치**: 그 Pareto front 위의 **점 3개를 연속 곡선으로 바꾼다.** RGB CNN 의
...
이 되어, **애초 목표였던 "RGB CNN 의 속도 장점"이 사라진다**. 정확도만 얻고 속도는 잃으며,
...
- [ ] ⚠️ **위 두 항목(게이트 품질)은 Phase 12 범위에서 제외됐다**(2026-08-05 결정).
  docs/11 은 에스컬레이션 비율 `r` 이 아니라 비용식의 다른 항인 `C_cnn`(다단 조기종료, M4)을
  다룬다. 두 축은 곱해지므로(docs/11 §5.2) **게이트 개선은 여전히 남은 최대 미착수 이득**이다.
- [ ] **대안 설계 — 지식 증류(knowledge distillation)**: char-CNN 을 교사로 RGB CNN 을
