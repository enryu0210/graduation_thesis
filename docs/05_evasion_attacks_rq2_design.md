...
#### ⚠️ 2026-09-15 보강 — 문헌 이름 정렬 (정본: docs/13 §1.13)

위 두 지표는 그대로 두되, **회피·방어 계열의 문헌 통화와 이름을 맞춘다.** 대조표에서 비교가
끊기지 않게 하기 위함이고, 측정 방식은 바뀌지 않는다.

| 우리 지표 | 문헌 이름 | 산출 키 |
|---|---|---|
| 주 지표 Benign-evasion | ✅ **ASR (Attack Success Rate)** | `asr_*` (**이미 이 키였다**) |
| 보조 지표 Any-misclassification | ❌ **ASR 이 아니다** — 공격끼리의 혼동은 WAF 가 여전히 차단한다 | `anymis_*` |
| **AUA (accuracy under attack)** 〔신설〕 | ✅ AUA — 변형 후 mask 내 정확도 = **1 − any-misclass** | `aua_*` |
| **AMB (최소 성공 예산)** 〔신설〕 | ⚠️ **AVGQ 아님** — 우리 변형은 질의 기반 탐색이 아니다 | `budget_summary` |

- ⚠️ docs/13 §1.9 발견 ②는 *"any-misclass 를 ASR 로 개명하라"* 고 적었으나 **틀린 권고였다**
  (코드를 보지 않고 내린 판정). 경위와 정정 근거는 **docs/13 §1.13**.
- ⚠️ **AMB 를 논문에서 AVGQ 라 부르지 말 것.** `mutate_stacked` 가 예산 k 마다 기법을 **독립
  샘플링**하므로 "k번째 질의에서 처음 성공"이라는 해석이 성립하지 않는다. 정의를 병기한다.
- AMB 가 필요한 이유는 **F7(비용 기반 공격)의 교환비**다 — 방어자 비용을 올리는 공격을
  주장하려면 **공격자 비용**도 함께 적어야 한다(docs/13 §1.10 (다)-4).
