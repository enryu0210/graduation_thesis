...
더 나은 탐지 성능을 보이는가?"* — 이를 검증하기 위해 **RGB CNN 1종 + 베이스라인 3종**을
...
# 2) RGB CNN + 텍스트 베이스라인 — torch 필요 (GPU 권장)
python src/models/train.py --model cnn        # RGB CNN(지표 비교 기준)
...
> GPU 전면 학습 완료(2026-07-18 재실측, RTX 4080 SUPER, seed=42). 전 모델을 **동일 test 셋·동일 지표
> 모듈**(`src/eval/metrics.py`)로 평가. 지표 원본: `experiments/results/*.json`, 혼동행렬: `docs/figures/models/`.
> Phase 7에서 MCC·PR-AUC 를 도입(docs/07 §3)했으나 베이스라인 4종은 도입 이전 산출물이었으므로,
> **동일 스키마 확보를 위해 4종을 재실행**해 아래 표를 통일했다.
> `thr/s` = test 셋 추론 처리량(samples/sec). benEv = benign-evasion(공격→Normal 누출, 낮을수록 좋음).

| # | 모델 | 입력 | Acc | Macro-F1 | **MCC** | **PR-AUC** | benEv | FPR | thr/s |
|---|---|---|---|---|---|---|---|---|---|
| 1 | TF-IDF + LogReg | 문자열 | 0.9765 | 0.9775 | 0.9688 | 0.9972 | 0.0282 | 0.0071 | 639,003 |
| 2 | TF-IDF + RandomForest | 문자열 | 0.9921 | 0.9924 | 0.9895 | 0.9992 | 0.0081 | 0.0047 | 93,572 |
| 3 | char-CNN | 바이트 시퀀스 | **0.9968** | **0.9969** | **0.9957** | 0.9995 | **0.0024** | 0.0047 | 6,010 |
| 4 | BiLSTM | 바이트 시퀀스 | 0.9960 | 0.9962 | 0.9946 | 0.9995 | 0.0037 | **0.0036** | 5,241 |
| 5 | gray CNN(1ch) | 48×48 이미지 | 0.9707 | 0.9714 | 0.9609 | 0.9914 | 0.0281 | 0.0200 | 92,466 |
| **6** | **RGB CNN rb/cc/bd** | **48×48×3 이미지** | **0.9832** | **0.9836** | **0.9775** | **0.9963** | **0.0131** | 0.0212 | 81,964 |

(RGB 채널 조합 4종의 상세 ablation 은 docs/07 §2.4 참조. 위 6번은 그중 MCC 최고 조합.)

### 해석 — RQ1에 대한 정직한 답

RQ1은 *"이미지화 CNN이 텍스트 모델과 **유사하거나 더 나은** 성능을 보이는가?"* 였다.
**clean in-distribution 기준으로는 "여전히 낮다"** 가 실측 결론이다.

- **RGB 강화는 얕은 CNN을 크게 끌어올렸다**: gray 대비 MCC 0.9609→0.9775(**+1.66pp**),
  benign-evasion 2.81%→1.31%(**절반 이하**). 교수 지시 ①의 전제("채널 다양화 = 특징 추출 이득")는
  실측으로 확인됐다.
- **그럼에도 순위는 6종 중 4위**다. char-CNN(MCC 0.9957)·BiLSTM(0.9946)·TF-IDF+RF(0.9895)에
  여전히 1.2~1.8pp 뒤진다. RGB로 이긴 상대는 TF-IDF+LogReg(0.9688) 하나뿐이다.
  → **"이미지화가 텍스트보다 정확하다"는 주장은 이 데이터에서 성립하지 않는다.** RGB는 격차를
  좁혔을 뿐 뒤집지 못했다.
- **제안 기법이 실제로 이기는 축은 정확도가 아니라 처리량**: RGB CNN 81,964 samples/s 는
  char-CNN(6,010)·BiLSTM(5,241) 대비 **13~16배** 빠르다. 정확도-속도 Pareto front 는
  LogReg(빠름·약함) → **RGB CNN(중간)** → char-CNN(느림·강함) 으로 형성되며, 제안 기법은
  이 front 위에 있다. WAF 인라인 배포처럼 지연이 제약인 환경에서의 실용 논거.
- ⚠️ **FPR 은 오히려 RGB가 최악(0.0212)**: gray(0.0200)보다도 나쁘고 텍스트 모델(0.0036~0.0071)의
  3~6배다. RGB는 공격 누출(benEv)은 줄였지만 정상 오탐은 줄이지 못했다. 대안 조합
  `rb/ss/le` 는 MCC 0.9770(-0.05pp, 사실상 동률)에 FPR 0.0138 로 **오탐이 35% 낮다** →
  운영 지표를 FPR로 잡는다면 최종 조합을 rb/ss/le 로 재검토할 것(docs/07 §2.4 연계).
...
- **재현성 실측(2026-07-18)**: 재실행 결과 TF-IDF 2종은 소수점까지 완전 일치, BiLSTM 도 일치.
  단 **char-CNN 만 Acc 0.9957→0.9968(+0.11pp) 변동** — 데이터·분할이 아니라 cuDNN 비결정적
  커널 탓으로 보인다. ⚠️ 이 변동폭(±0.1pp)은 RGB 조합 간 차이(rb/cc/bd vs rb/ss/le 의 MCC 0.05pp)
  **보다 크다** → 조합 우열은 단일 split으로 단정 불가. 6절 5-fold CV 가 필수인 직접 근거.
- 통계 검증(5-fold CV + paired t-test)은 **5.1 절에서 완료** — 위 표는 단일 split(seed=42) 결과다.

---

## 5.1 5-fold 교차검증 결과 (2026-07-18) — RQ1 최종 판정

> `src/eval/cross_validate.py`(풀 199,793개 → StratifiedKFold k=5, seed=42) +
> `src/eval/cv_compare.py`(paired t-test, Holm-Bonferroni 보정).
> 모든 설정이 **동일 fold** 를 봤음을 `label_fingerprint` 로 검증(대응표본 전제).
> 원본: `experiments/results/cv_*.json`, 그림: `docs/figures/models/cv_ranking_mcc.png`.

| 순위 | 설정 | MCC (5-fold) | benign-evasion |
|---|---|---|---|
| 1 | **char-CNN** | **0.9944 ± 0.0004** | 0.0038 ± 0.0010 |
| 2 | TF-IDF + RF | 0.9889 ± 0.0009 | 0.0086 ± 0.0007 |
| 3 | RGB CNN rgb rb/cc/le | 0.9762 ± 0.0025 | 0.0148 ± 0.0029 |
| 4 | RGB CNN rgb rb/cc/bd | 0.9761 ± 0.0051 | 0.0163 ± 0.0053 |
| 5 | RGB CNN rgb rb/ss/le | 0.9723 ± 0.0062 | 0.0185 ± 0.0050 |
| 6 | RGB CNN rgb rb/bp/le | 0.9718 ± 0.0034 | 0.0167 ± 0.0025 |
| 7 | TF-IDF + LogReg | 0.9686 ± 0.0011 | 0.0290 ± 0.0011 |
| 8 | gray CNN | 0.9493 ± 0.0109 | 0.0310 ± 0.0055 |

(BiLSTM 은 계산 비용 대비 정보량이 낮아 제외 — 단일 split 에서 char-CNN 에 뒤졌고,
"RGB CNN vs 텍스트 베이스라인 최강자" 비교는 char-CNN 으로 충분히 판정된다.)

### 통계적으로 확정된 것 / 확정 안 된 것

**① 확정 — RGB CNN 은 텍스트 베이스라인에 유의하게 열세다.**
char-CNN 은 RGB CNN 의 **모든 RGB 조합보다 유의 우세**(adj p = 0.0010 ~ 0.0247),
TF-IDF+RF 도 rgb 기본 조합보다 유의 우세(adj p = 0.0093). char-CNN > TF-IDF+RF 역시 유의(adj p = 0.0075).
→ **"이미지화가 텍스트보다 정확하다"는 주장은 통계적으로 기각된다.** 단일 split 의 순위가
그대로 유지됐고, 이제는 우연이 아님이 확인됐다.

**② 확정 — gray → RGB 개선은 진짜다.**
gray 기준 7쌍 비교에서 gray 는 RGB 4조합 **전부에 유의하게 열세**(adj p = 0.0037 ~ 0.0201).
benign-evasion 도 3.10% → 1.48% 로 유의하게 감소(adj p = 0.0194). 교수 지시 ①의 성과.

**③ 기각 — RGB 조합 간 우열은 판정 불가.**
4조합 6쌍 비교가 **전부 판정불가**(Holm 보정 후 adj p ≥ 0.88). 특히 rb/cc/le vs rb/cc/bd 는
**p = 0.976** 으로 사실상 동일하다. docs/07 §2.4 의 "rb/cc/bd 최고", "bit_popcount 제외"
결론은 **철회**했다(docs/07 §2.5). → 기본 조합 `rb/cc/le` 를 문헌 근거 우위로 유지.

**④ 주의 — 검정력 한계.** fold=5 는 자유도 4로 검정력이 낮다. "판정불가"는 "차이 없음"이
아니라 "이 표본수로는 구분 못 함"이다. 조합 우열을 주장하려면 반복 CV(5×2, 10-fold)가 필요하다.

### 제안 기법의 방어 가능한 논거는 무엇으로 남는가

정확도 경쟁에서는 졌다(①). 남는 것은 **처리량**이다 — RGB CNN 81,964 samples/s 대
char-CNN 6,010 samples/s(13.6배). 정확도-속도 Pareto front 위에 있다는 점, 그리고 파라미터가
93,988 로 char-CNN(141,252)보다 작다는 점이 실용 논거다. 논문은 이 축으로 서술하고,
정확도 열세는 숨기지 말고 명시한다.
...
- [x] 통계 검증: 5-fold CV + paired t-test(설계 6.1) — **완료(5.1절)**. RGB CNN 의 텍스트
      베이스라인 대비 열세가 통계적으로 확정됐고, RGB 조합 간 우열은 판정불가로 기각.
      ⚠️ 남은 보강: 검정력을 위한 반복 CV(5×2 또는 10-fold), BiLSTM 미실행(비용 대비 정보량 낮음)
...
## 8. 탐지 상보성 분석 — "RGB CNN 이 남들이 놓친 공격을 잡는가?" (실측, 2026-07)
...
- 대상: `payload_4class_csicnorm` test 의 **진짜 공격 21,907 건**. 기준(ref)=RGB CNN.
...
- **함의**: 이미지화 표현은 텍스트 베이스라인 대비 **탐지 상보성에서 이득이 없다**(clean 기준).
  → RGB CNN 의 존재 이유는 clean 탐지 우월성이 아니라 **RQ2(회피 강건성)** 에서 찾아야 한다는
  5·7절 결론을 재확인한다.
